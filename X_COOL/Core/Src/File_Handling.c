...
extern FIL USERFile; 		//File handle
FILINFO USBHfno;
...
FRESULT copy_file(char* dest ,char* src)
{
	 ...
	 fr = f_open(&USERFile, src, FA_READ);
	 if (fr) return (int)fr;

	 /* Create destination file on the drive 0 */
	fr = f_open(&USBHFile, dest, FA_WRITE | FA_CREATE_ALWAYS);
	if (fr) return (int)fr;

	/* Copy source to destination */
	for (;;) {
		fr = f_read(&USERFile, usb_buffer, MAX_USB_BUFFER , &br);  /* Read a chunk of source file */
		if (fr || br == 0) break; /* error or eof */
		fr = f_write(&USBHFile, usb_buffer, br, &bw);            /* Write it to the destination file */
